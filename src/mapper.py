...
def vis_render_process(gaussians, pipeline_params, background, viewpoint, cur_frame_idx, save_dir, out_dir="map",
                       mask=None, dynamic=False, save_debug=False):
...
class Mapper(object):
    ...
    def __init__(self, slam, pipe: Connection):
        ...
        self.gaussians = GaussianModel(model_params.sh_degree, config=self.config, args=hp, init_deform=self.dynamic_model)
    ...
    def set_hyperparams(self):
        ...
        self.save_debug_visualization = self.config.get('save_debug_visualization', False)
    ...
    def get_surface_normalv2(self, xyz, patch_size=3):
        ...
        xyz_normal = torch.nn.functional.pad(xyz_normal, (0, 0, 1, 1, 1, 1), mode="constant")
    ...
    def initialize_map(self, cur_frame_idx, idx, viewpoint):
        ...
        pbar = tqdm(range(self.init_itr_num), desc=prefix + "Initializing map", 
        ...
        for mapping_iteration in pbar:
            ...
            render_pkg = self._render_viewpoint(viewpoint, 0, 0, None, None, None, return_normal=False)
            image, viewspace_point_tensor, visibility_filter, radii, depth, opacity, n_touched, normal = self._unpack_render_pkg(render_pkg)
            ...
            if flatten_w > 0 and normal is not None:
                loss_init += flatten_w * self.get_loss_flattening(normal, viewpoint, opacity=opacity)
            if flatten_w > 0:
                loss_init += self.scale_loss_weight * self.get_loss_flat(visibility_filter)
            loss_init.backward()
            ...
            with torch.no_grad():
                ...
                if mapping_iteration % self.init_gaussian_update == 0:
                    self.gaussians.densify_and_prune(
                        self.opt_params.densify_grad_threshold, self.init_gaussian_th,
                        self.init_gaussian_extent, None,
                    )
    ...
    def _get_deform_d_values(self, viewpoint, use_noise=False, detach_outputs=False, iteration=0):
        ...
        if detach_outputs:
            ...
            d_opac = d_values["d_opacity"].detach() if d_values.get("d_opacity") is not None else None
            d_color = d_values["d_color"].detach() if d_values.get("d_color") is not None else None
        else:
            dxyz = d_values["d_xyz"]
    ...
    def _render_viewpoint(self, viewpoint, dxyz, d_scale, d_rot, d_opac=None, d_color=None, return_normal=False):
    ...
    def _compute_flow_loss_pair(self, viewpoint, closest_viewpoint, idx1, dxyz, d_rot, d_scale, flow_weights):
        ...
        loss_fwd = flow_weights * l1_loss(flow_back * dynamic_mask, coor1to2_motion * dynamic_mask)
        ...
        dynamic_mask_b = (~closest_viewpoint.motion_mask).unsqueeze(0).permute(1, 2, 0).repeat(1, 1, 2).detach()
    ...
    def _add_deform_regularization_losses(self, viewpoint, delta=None, scale_arap=1e-3, scale_acc=1e-5, scale_elastic=1e-3):
        ...
        loss = scale_arap * self.gaussians.deform.deform.arap_loss(
            t=viewpoint.fid, delta_t=delta_t, t_samp_num=4,
        )
        loss = loss + scale_acc * self.gaussians.deform.deform.acc_loss(
            t=viewpoint.fid, delta_t=5 * self.gaussians.time_interval,
        )
    ...
    def map(self, stream, idx1, current_window, prune=False, iters=1, dynamic_network=False, dynamic_render=False, rm_initdy=False):
        ...
        from src.utils.Printer import get_msg_prefix
        prefix = get_msg_prefix(FontColor.MAPPER)
        pbar = tqdm(range(iters), desc=prefix + "Mapping",
                    bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]',
                    position=0, leave=True)
        flatten_w = self.flattening_loss_weight
        if flatten_w > 0 and self.rotation_lr is not None:
        ...
        if getattr(self, "profile_mapping_time", False):
            t_acc = {"deform": 0.0, "render": 0.0, "flow": 0.0, "mapping_loss": 0.0, "multi_view": 0.0, "reg": 0.0, "backward": 0.0, "optim": 0.0}
            t_count = 0
        ...
        for i in pbar:
            ...
            if i < iters / 2:
                ...
                dynamic = False
                flow_weights = self.config["mapping"]["Training"].get("flow_loss_fine", self.config["mapping"]["Training"]["flow_loss"])
            ...
            for cam_idx in range(num_views):
                ...
                if dynamic_network and self.gaussians.deform_init:
                    ...
                    with torch.no_grad():
                        dxyz, d_rot, d_scale, d_opac, d_color = self._get_deform_d_values(
                            viewpoint, use_noise=True, detach_outputs=True, iteration=0,
                        )
                else:
                    dxyz = 0
                ...
                render_pkg = self._render_viewpoint(viewpoint, dxyz, d_scale, d_rot, d_opac, d_color, return_normal=need_normal)
                if self.profile_mapping_time:
                    torch.cuda.synchronize()
                ...
                if dynamic_network and self.gaussians.deform_init:
                    ...
                    if closest_keyframe is not None and closest_keyframe in self.viewpoints:
                        ...
                        flow_loss_delta, d_value2 = self._compute_flow_loss_pair(
                            viewpoint, closest_viewpoint, idx1, dxyz, d_rot, d_scale, flow_weights,
                        )
                    ...
                    if flatten_w > 0 and normal is not None:
                        loss_mapping += flatten_w * self.get_loss_flattening(normal, viewpoint, opacity=opacity)
                    if closest_keyframe is not None and closest_keyframe in self.viewpoints and i > iters * 0.8:
                        loss_mapping += self.compute_multi_view_loss(
                            viewpoint, render_pkg, self.gaussians,
                    ...
                    loss_network += self._add_deform_regularization_losses(viewpoint, delta=delta)
                else:
                    loss_mapping += get_loss_mapping(
                        self.config["mapping"], image, depth, viewpoint, opacity,
                        rm_dynamic=not (dynamic_network or dynamic_render),
            ...
            with torch.no_grad():
                for idx in range(len(keyframes_opt)):
                    ...
                    visibility_filter_cache[kf_idx] = visibility_filter_acm[idx].detach()
                for idx in range(len(viewspace_point_tensor_acm)):
                    ...
                    self.gaussians.add_densification_stats(viewspace_point_tensor_acm[idx].detach(), vf)
                update_gaussian = (
                    self.iteration_count % self.gaussian_update_every == self.gaussian_update_offset and i > 100
                )
                if rm_initdy:
                ...
                if update_gaussian:
                    ...
                    gaussian_split = True
                if (self.iteration_count % self.gaussian_reset) == 0 and (not update_gaussian) and i > 100:
                    self.printer.print("Resetting the opacity of non-visible Gaussians", FontColor.MAPPER)
                    visibility_filter_for_reset = [visibility_filter_cache[k] for k in current_window if k in visibility_filter_cache]
                    if len(visibility_filter_for_reset) > 0:
                        self.gaussians.reset_opacity_nonvisible(visibility_filter_for_reset)
                    gaussian_split = True
                ...
                for cam_idx in range(min(frames_to_optimize, len(current_window))):
                    ...
                    if vp.uid == 0:
                        continue
                    update_pose(vp)
            if dynamic_network and self.gaussians.deform_init and isinstance(loss_network, torch.Tensor) and loss_network.requires_grad:
        ...
        if prune and len(current_window) == self.window_size:
            with torch.no_grad():
                for kf_idx in current_window:
                    ...
                    if vp is None:
                        continue
                    render_pkg = self._render_viewpoint(vp, 0, 0, None, None, None, return_normal=False)
                    _, _, visibility_filter, _, _, _, n_touched, _ = self._unpack_render_pkg(render_pkg)
    ...
    def final_refine(self, prune=False, iters=26000):
        ...
        from src.utils.Printer import get_msg_prefix
        prefix = get_msg_prefix(FontColor.MAPPER)
        pbar = tqdm(range(iters), desc=prefix + "Final refinement", 
                   bar_format='{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]', 
                   position=0, leave=True)
        flatten_w = self.flattening_loss_weight
        if flatten_w > 0 and self.rotation_lr is not None:
        ...
        for iteration in pbar:
            ...
            for _ in range(10):
                ...
                render_pkg = self._render_viewpoint(
                    viewpoint, dxyz, d_scale, d_rot, d_opac, d_color, return_normal=(flatten_w > 0)
                )
                (image, viewspace_point_tensor, visibility_filter, radii, depth, opacity, n_touched, normal) = self._unpack_render_pkg(render_pkg)
                ...
                if flatten_w > 0 and normal is not None:
                    loss += flatten_w * self.get_loss_flattening(normal, viewpoint, opacity=opacity)
                if self.dynamic_model:
                ...
                if getattr(self, 'save_debug_visualization', False) and iteration % 400 == 0 and iteration > 7000:
                ...
                if iteration % 1000 == 0 and iteration > 0:
                    try:
                        ...
                        for kf_idx, video_idx in zip(self.keyframe_idxs, self.video_idxs):
                            ...
                            if self.dynamic_model and self.gaussians.deform_init:
                                dxyz, d_rot, d_scale, d_opac, d_color = self._get_deform_d_values(
                                    frame, use_noise=False, detach_outputs=False, iteration=0,
                                )
                            else:
                                dxyz, d_rot, d_scale, d_opac, d_color = 0, 0, 0, None, None
                            render_pkg = self._render_viewpoint(frame, dxyz, d_scale, d_rot, d_opac, d_color)
            ...
            #print("eval remove motion region")
                ...
                if getattr(self, 'save_debug_visualization', False) and iteration % 1000 == 0 and iteration > 0:
                    try:
                        ...
                        for kf_idx, video_idx in zip(self.keyframe_idxs, self.video_idxs):
                            ...
                            if self.dynamic_model and self.gaussians.deform_init:
                                try:
                                    ...
                                    if closest_keyframe is not None and closest_keyframe in self.viewpoints:
                                        closest_viewpoint = self.viewpoints[closest_keyframe]
                                        d_xyz1, d_rot1, d_scale1, _, _ = self._get_deform_d_values(
                                            frame, use_noise=False, detach_outputs=False, iteration=0,
                                        )
                                        d_value2 = self._get_deform_d_values_for_fid(
                                            closest_viewpoint.fid, closest_viewpoint.camera_center, iteration=0,
                                        )
                                        d_xyz2, d_rot2, d_scale2 = d_value2["d_xyz"], d_value2["d_rotation"], d_value2["d_scaling"]
                                        flow_pkg = render_flow(
                                            pc=self.gaussians,
                                            viewpoint_camera1=frame,
                                            viewpoint_camera2=closest_viewpoint,
                                            d_xyz1=d_xyz1, d_xyz2=d_xyz2,
                                            d_rotation1=d_rot1, d_scaling1=d_scale1
                                        )
                                        if "render" in flow_pkg:
                                            flow = flow_pkg["render"].detach().cpu().numpy()
    ...
    def get_loss_flat(self, visibility_filter=None):
        ...
        if visibility_filter is not None and visibility_filter.shape[0] == scaling.shape[0]:
    ...
    def get_loss_flattening(self, normal, viewpoint, opacity=None, opacity_threshold=0.95):
        ...
        """
        rendered_n = torch.nn.functional.normalize(normal.permute(1, 2, 0), dim=-1)  # (H, W, 3)
        ...
        image = viewpoint.original_image.to(device=rendered_n.device, dtype=rendered_n.dtype)
    ...
    def run(self,stream:BaseDataset):
        ...
        while (1):
            ...
            data = {"gt_color": color.squeeze(), "glorie_depth": depth.cpu().numpy(), "glorie_pose": w2c, "gt_pose": w2c_gt, "idx": video_idx}
            viewpoint = Camera.init_from_dataset(self.frame_reader, video_idx, idx, data, projection_matrix)
            viewpoint.update_RT(viewpoint.R_gt, viewpoint.T_gt)
            viewpoint.compute_grad_mask(self.config)
            self.cameras[video_idx] = viewpoint
            if self.dynamic_model:
            ...
            dxyz, d_rot, d_scale = 0, 0, 0
            render_pkg = render(viewpoint, self.gaussians, self.pipeline_params, self.background, dynamic=False, dx=dxyz, ds=d_scale, dr=d_rot)
            ...
            if create_kf:
                self.current_window, removed = self.add_to_window(video_idx, curr_visibility, self.occ_aware_visibility, self.current_window)
            ...
            self.map(stream=stream, idx1=idx, current_window=self.current_window, prune=True, dynamic_network=dynamic_enabled)
