...
class BaseDataset(Dataset):
    def __init__(self, cfg, device='cuda:0'):
        ...
        self.motion_mask_255_is_static = cfg.get("motion_mask_255_is_static", cfg.get("data", {}).get("motion_mask_255_is_static", True))
        # 是否使用 Depth Anything 预测深度替代真值深度（data.use_depth_anything 或顶层 use_depth_anything）
        self.use_depth_anything = cfg.get("use_depth_anything", cfg.get("data", {}).get("use_depth_anything", False))
        ...
        if cfg.get("data") and cfg["data"].get("output") and cfg.get("scene"):
            self.mono_priors_dir = os.path.join(cfg["data"]["output"], cfg["scene"], "mono_priors", "depths")
        # YOLO 权重路径，默认 pretrained/yolo11l-seg.pt（与 download_pretrained.sh 一致）
        self.yolo_pretrained = cfg.get("data", {}).get("yolo_pretrained", "pretrained/yolo11l-seg.pt")
    ...
    def depth_to_normal(self, depth, smooth_depth=True, use_median_filter=False, k_scale=1.0):
        ...
        if smooth_depth:
            if use_median_filter:
                depth_np = depth.cpu().numpy()
                depth_np = cv2.medianBlur(depth_np.astype(np.float32), 3)
                depth_4d = torch.from_numpy(depth_np).float().to(dev).unsqueeze(0).unsqueeze(0)
            else:
                gaussian = torch.tensor(
                    [[1, 2, 1], [2, 4, 2], [1, 2, 1]], dtype=torch.float32, device=dev
                ) / 16.0
        ...
        valid = depth > 1e-6
        valid = valid & torch.roll(valid, 1, 0) & torch.roll(valid, -1, 0) & torch.roll(valid, 1, 1) & torch.roll(valid, -1, 1)
        valid[0, :] = False
        valid[-1, :] = False
        valid[:, 0] = False
        valid[:, -1] = False

        normal_hwc = torch.where(valid.unsqueeze(-1), normal_hwc, torch.zeros_like(normal_hwc))
    ...
    def __getitem__(self, index):
        ...
        if getattr(self, "use_depth_anything", False):
            ...
            if getattr(self, "mono_priors_dir", None) and os.path.isfile(os.path.join(self.mono_priors_dir, f"{index:05d}.npy")):
            ...
            if os.path.isfile(path):
                ...
                if img is not None:
                    ...
                    if getattr(self, "motion_mask_255_is_static", False):
                        ...
                        mask_from_disk = (img <= 127).astype(np.float32)
                    mask_from_disk = torch.from_numpy(mask_from_disk).unsqueeze(0).unsqueeze(0)
                    mask_from_disk = F.interpolate(
                        mask_from_disk, size=(self.H_out, self.W_out), mode="nearest"
                    ).squeeze().bool().to(self.device)
        
        if mask_from_disk is not None:
            final_mask = mask_from_disk  # True=静态
        else:
            final_mask = torch.ones((self.H_out, self.W_out), device=self.device, dtype=torch.bool)
...
class CoFusion(BaseDataset):
    def __init__(self, cfg, device='cuda:0'):
        ...
        self.color_paths, self.depth_paths, self.poses, self.mask_paths, self.normal_paths, self.mono_paths, self.static_paths = self.load_cofusion(
            self.input_folder)
    ...
    def load_cofusion(self, datapath):
        ...
        # Load color images
        color_paths = sorted(
            glob.glob(os.path.join(datapath, 'colour', '*.png')))
        # Load depth images
        depth_paths = sorted(
        ...
        for mask_dir in mask_dirs:
            ...
            if os.path.isdir(mask_dir_path):
                mask_paths = sorted(glob.glob(os.path.join(datapath, mask_dir, "*.png")),
                                   key=self.extract_number)
                if mask_paths:  # 确保找到了文件
                    print(f"✅ Using mask images from: {mask_dir}/ ({len(mask_paths)} files)")
                    break
        if not mask_paths:
        ...
        if os.path.isdir(os.path.join(datapath, "normal")):
            normal_paths = sorted(glob.glob(os.path.join(datapath, "normal", "*.npy")),
                                 key=self.extract_number)
        ...
        if os.path.isdir(os.path.join(datapath, "depth_npy")):
            mono_paths = sorted(glob.glob(os.path.join(datapath, "depth_npy", "*.npy")),
                               key=self.extract_number)
        ...
        if os.path.isdir(os.path.join(datapath, "flow_i1")):
            static_paths = sorted(glob.glob(os.path.join(datapath, "flow_i1", "*.npz")),
                                 key=self.extract_number)
        ...
        mask_paths = mask_paths[:min_length] if mask_paths[0] is not None else [None] * min_length
        normal_paths = normal_paths[:min_length] if normal_paths else [None] * min_length
        mono_paths = mono_paths[:min_length] if mono_paths else [None] * min_length
        static_paths = static_paths[:min_length] if static_paths else [None] * min_length
        
        print(f"Loaded {min_length} frames from CoFusion dataset")
        
        return color_paths, depth_paths, poses, mask_paths, normal_paths, mono_paths, static_paths
