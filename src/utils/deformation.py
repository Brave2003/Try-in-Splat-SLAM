...
class pose_network(nn.Module):
    def __init__(self, dataset=None):
        ...
        if dataset is not None:
            ...
            print("self.instance_scale_list",self.instance_scale_list)
            # self.instance_shift_list = nn.Parameter(torch.cat(self.instance_shift_list, 0), requires_grad=True)
            self.max_time = 1.0
            self.H = dataset.H_out
            self.W = dataset.W_out
            print("posenet_init success")
    def forward(self, times_sel, depth=None):
        ...
        print("cannonical_scale",cannonical_scale)
