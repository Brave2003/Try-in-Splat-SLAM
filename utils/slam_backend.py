...
def vis_render_process(gaussians, pipeline_params, background, viewpoint, cur_frame_idx, save_dir, out_dir="map", mask=None, dynamic=False):
    with torch.no_grad():
        ...
        render_pkg = render(
            viewpoint, gaussians, pipeline_params, background, dynamic=False, dx=dxyz, ds=d_scale, dr=d_rot
        )
        #render_pkg = render(
...
class BackEnd(mp.Process):
    ...
    def map(self, current_window, prune=False, iters=1, dynamic_network=False, dynamic_render=False, rm_initdy=False):
        if len(current_window) == 0:
            return
        # current_window[:3]和5个随机重叠关键帧作为固定map帧
        key_opt = []
        ...
        for i in range(iters):
            ...
            for cam_idx in range(len(current_window)):
                ...
                if dynamic_network and self.gaussians.deform_init:
                    ...
                    if closest_keyframe is not None:# and i<iters*3/4:
                        ...
                        loss_network += flow_weights*l1_loss(flow*dynamic_mask, coor2to1_motion*dynamic_mask)  # 是否只在前150次迭代才加入？？
            ...
            #dygs_isotropic_loss = torch.abs(dygs_scaling - dygs_scaling.mean(dim=1).view(-1, 1))
            #loss_mapping += 10 * dygs_isotropic_loss.mean()
            ...
            with torch.no_grad():
                ...
                if prune:
                    ...
                    #           self.viewpoints[current_window[0]].uid, self.save_dir, out_dir="map", mask=None, dynamic=(dynamic_network and self.gaussians.deform_init))
                ...
                if rm_initdy:
                    update_gaussian = (iters - i-10 == 0)  # 优化静态部分时，在结束前十帧前进行denstify and prune 高斯
        ...
        if not (dynamic_network or dynamic_render):
            vis_render_process(self.gaussians, self.pipeline_params, self.background, self.viewpoints[current_window[0]], 
                               self.viewpoints[current_window[0]].uid, self.save_dir, out_dir="track", mask=None, dynamic=(dynamic_network and self.gaussians.deform_init))
    ...
    def color_refinement(self, dynamic_network=False):
        ...
        for iteration in tqdm(range(1, iteration_total + 1)):
            ...
            with torch.no_grad():
                ...
                #self.gaussians.update_learning_rate_deformation(iteration)
                if dynamic_network and self.gaussians.deform_init:
                    self.gaussians.deform.optimizer.step()
                    self.gaussians.deform.optimizer.zero_grad(set_to_none=True)
                    #self.gaussians.deform.update_learning_rate(iteration)
                ...
                if prune:
                    if len(current_window) == self.config["Training"]["window_size"]:
                        ...
                        if prune_mode == "slam":
                            # only prune keyframes which are relatively new
                            sorted_window = sorted(current_window, reverse=True)
                            mask = self.gaussians.unique_kfIDs >= sorted_window[2]  # 只prune最近两关键帧的高斯
                            if not self.initialized:
                                mask = self.gaussians.unique_kfIDs >= 0
                            to_prune = torch.logical_and(
                                self.gaussians.n_obs <= prune_coviz, mask  # prune作用像素小于3的高斯
                            )
                        if to_prune is not None and self.monocular:
                            self.gaussians.prune_points(to_prune.cuda())
                            for idx in range((len(current_window))):
